...
namespace JunoSDK
{
	public class JunoModule
	{
		...
		public static JunoModule FromUnixSocket(string socketPath)
		{
			if (Misc.IsUnix)
			{
				return new JunoModule(new JsonProtocol(), new UnixSocketConnection(socketPath));
			}

			throw new ApplicationException("Unix sockets are not supported on windows!");
		}

		public static JunoModule FromInetSocket(string address, short port)
		{
			return new JunoModule(new JsonProtocol(), new InetSocketConnection(address, port));
		}
		...
		public async Task Initialize(string moduleId, string version, Dictionary<string, string> dependencies = null)
		{
